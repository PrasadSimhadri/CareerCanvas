...
export default function SidebarTemplate({ data }: { data: PortfolioData }) {
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', background: colors.bg, color: colors.text, fontFamily: "'Inter', sans-serif" }}>
            {/* Fixed Sidebar */}
            <aside className="hidden md:flex" style={{ position: 'fixed', left: 0, top: 0, width: 260, height: '100vh', background: colors.sidebarBg, borderRight: '1px solid rgba(245,158,11,0.1)', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', zIndex: 100, overflow: 'auto' }}>
                {/* Profile Photo */}
                    ...
                    {navItems.map((item) => (
                        <button
                            ...
                            style={{
                                ...
                                color: activeSection === item.id ? '#F59E0B' : colors.sidebarTextOff,
                            }}
                            onMouseEnter={(e) => {
                            ...
                            onMouseLeave={(e) => {
                                if (activeSection !== item.id) {
                                    ...
                                    e.currentTarget.style.color = colors.sidebarTextOff;
                                }
                            }}
                        >
            ...
            <main style={{ flex: 1, padding: '40px 5%' }} className="md:ml-[260px]">
                ...
                <section id="about">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        ...
                        <p style={{ color: colors.textSec, fontSize: '1.05rem', lineHeight: 1.8, marginBottom: 32, maxWidth: 700 }}>{data.basicInfo.description}</p>
                    </motion.div>

                    {data.about.description && (
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                            <SectionHeading>About</SectionHeading>
                            <div style={{ background: colors.cardBg, borderRadius: 16, border: '1px solid rgba(245,158,11,0.1)', padding: 32, marginBottom: 32 }}>
                                {data.about.description.split('\n').map((p, i) => (
                                    <p key={i} style={{ color: colors.textSec, lineHeight: 1.8, marginBottom: 12 }}>{p}</p>
                                ))}
                                {data.about.interests?.length > 0 && (
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
                                        {data.about.interests.map((interest, i) => (
                                            <span key={i} style={{ background: colors.cardBorder, border: '1px solid rgba(245,158,11,0.15)', color: '#FCD34D', padding: '6px 14px', borderRadius: 8, fontSize: '0.8rem', fontWeight: 500 }}>{interest}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
                    ...
                    {data.about.cards?.length > 0 && (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginBottom: 48 }}>
                            {data.about.cards.map((card, i) => (
                                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 24 }}>
                                    <h4 style={{ color: '#F59E0B', fontWeight: 700, marginBottom: 8, fontSize: '1rem' }}>{card.title}</h4>
                                    <p style={{ color: colors.sidebarTextOff, fontSize: '0.85rem', lineHeight: 1.6 }}>{card.description}</p>
                                </motion.div>
                            ))}
                        </div>
                    )}
                    ...
                    <section id="experience" style={{ marginBottom: 48 }}>
                        ...
                        {data.experience.map((exp, i) => (
                            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} style={{ display: 'flex', gap: 16, marginBottom: 24 }}>
                                ...
                                <div style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 24, flex: 1 }}>
                                    ...
                                    <p style={{ color: colors.sidebarTextOff, fontSize: '0.8rem', marginBottom: 12 }}>{exp.startDate} — {exp.endDate} • {exp.location}</p>
                                    {exp.skills.length > 0 && (
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                                            {exp.skills.map((s, j) => (
                                                <span key={j} style={{ background: colors.cardBorder, color: '#FCD34D', padding: '3px 10px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 500 }}>{s}</span>
                                            ))}
                                        </div>
                                    )}
                                    <p style={{ color: colors.textSec, lineHeight: 1.7, fontSize: '0.9rem' }}>{exp.description}</p>
                                </div>
                            </motion.div>
                        ))}
                    </section>
                    ...
                    <section id="education" style={{ marginBottom: 48 }}>
                        ...
                        <div style={{ display: 'grid', gap: 16 }}>
                            {data.education.map((edu, i) => (
                                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 24 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                                        <div>
                                            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: 4 }}>{edu.institution}</h3>
                                            <p style={{ color: colors.sidebarTextOff }}>{edu.degree} • {edu.location}</p>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <p style={{ color: '#F59E0B', fontWeight: 600, fontSize: '0.85rem' }}>{edu.startYear} — {edu.endYear}</p>
                                            {edu.grade && <p style={{ color: colors.sidebarTextOff, fontSize: '0.85rem' }}>{edu.grade}</p>}
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
                    ...
                    <section id="projects" style={{ marginBottom: 48 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
                            {data.projects.map((proj, i) => (
                                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 24, transition: 'all 0.3s' }}>
                                    <h3 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: 10 }}>{proj.title}</h3>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                                        {proj.techStack.map((tech, j) => (
                                            <span key={j} style={{ background: colors.cardBorder, color: '#FCD34D', padding: '4px 10px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 500 }}>{tech}</span>
                                        ))}
                                    </div>
                                    <p style={{ color: colors.textSec, fontSize: '0.9rem', lineHeight: 1.7, marginBottom: 16 }}>{proj.description}</p>
                    ...
                    <section id="skills" style={{ marginBottom: 48 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
                            {data.skills.map((group, i) => (
                                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 20 }}>
                                    <h4 style={{ color: '#F59E0B', fontWeight: 700, fontSize: '0.95rem', marginBottom: 14, textAlign: 'center' }}>{group.category}</h4>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {group.skills.map((skill, j) => (
                                            <div key={j} style={{ padding: '8px 12px', background: 'rgba(245,158,11,0.05)', borderRadius: 8, fontSize: '0.85rem', color: colors.textSec }}>{skill}</div>
                                        ))}
                                    </div>
                                </motion.div>
                            ))}
                ...
                {data.achievements?.length > 0 && data.achievements[0].title && (
                    <section id="achievements" style={{ marginBottom: 48 }}>
                        ...
                        {data.achievements.map((ach, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 24, marginBottom: 16, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                                ...
                                <div>
                                    ...
                                    <p style={{ color: colors.textSec, fontSize: '0.9rem', lineHeight: 1.6 }}>{ach.description}</p>
                                </div>
                            </motion.div>
                        ))}
                    </section>
                ...
                <section id="contact" style={{ marginBottom: 48 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 16 }}>
                        {data.contact.email && <ContactCard colors={colors} icon={<FaEnvelope />} title="Email" value={data.contact.email} />}
                        {data.contact.phone && <ContactCard colors={colors} icon={<FaPhone />} title="Phone" value={data.contact.phone} />}
                        {data.contact.linkedinUrl && <ContactCard colors={colors} icon={<FaLinkedinIn />} title="LinkedIn" value={data.contact.linkedinUrl} />}
                        {data.contact.githubUrl && <ContactCard colors={colors} icon={<FaGithub />} title="GitHub" value={data.contact.githubUrl} />}
                ...
                <footer style={{ padding: '24px 0', textAlign: 'center', borderTop: '1px solid rgba(245,158,11,0.08)' }}>
                    <p style={{ color: colors.sidebarTextOff, fontSize: '0.85rem' }}>
                        Built with <a href="/" style={{ color: '#F59E0B', textDecoration: 'none', fontWeight: 700 }}>CareerCanvas</a>
                        {' • '}
                        <a href="https://github.com" target="_blank" style={{ color: colors.sidebarTextOff, textDecoration: 'none' }}><FaGithub style={{ display: 'inline', marginRight: 4 }} />GitHub</a>
                    </p>
                </footer>
...
function SidebarSocial({ href, icon, colors }: { href: string; icon: React.ReactNode; colors: any }) {
    return (
        <a href={href} target="_blank" style={{ width: 36, height: 36, borderRadius: '50%', background: colors.cardBorder, border: '1px solid rgba(245,158,11,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F59E0B', textDecoration: 'none', fontSize: '0.9rem', transition: 'all 0.2s' }}>{icon}</a>
    );
}

function ContactCard({ icon, title, value, colors }: { icon: React.ReactNode; title: string; value: string; colors: any }) {
    return (
        <div style={{ background: colors.cardBg, borderRadius: 14, border: '1px solid rgba(245,158,11,0.08)', padding: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: colors.cardBorder, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F59E0B', fontSize: '1rem', flexShrink: 0 }}>{icon}</div>
            <div>
                <p style={{ fontSize: '0.75rem', color: colors.sidebarTextOff, marginBottom: 2 }}>{title}</p>
                <p style={{ fontSize: '0.85rem', color: colors.textSec, wordBreak: 'break-all' }}>{value}</p>
            </div>
        </div>
    );
}
